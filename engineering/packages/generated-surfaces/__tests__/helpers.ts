...
export function graphOf(nodes: LogicNode[], connections: Connection[]): ProductGraph {
  return {
    schemaVersion: CURRENT_SCHEMA_VERSION,
    id: 'graph_surfaces_test',
    name: 'Test Rover',
    nodes,
    connections,
    userMode: 'builder',
...
/** The Motion Starter slice, already propagated so metrics are populated. */
export function roverGraph(): ProductGraph {
  ...
  return propagate(graphOf(nodes, connections)).graph;
}

/** A pack with nothing connected to it, so its runtime cannot be resolved. */
export function batteryOnlyGraph(): ProductGraph {
  return propagate(graphOf([
    node('n1_battery', 'battery', {
      chemistry: 'nimh', cellCount: 3, capacityMah: 2000, dischargeRating: 5,
    }),
  ], [])).graph;
...
/** A product with no wireless link of any kind. */
export function unlinkedGraph(): ProductGraph {
  const nodes = [
    node('n1_battery', 'battery', {
      chemistry: 'nimh', cellCount: 3, capacityMah: 2000, dischargeRating: 5,
    }),
    node('n2_controller', 'controller', { controller: 'rp2040' }),
  ];
  return propagate(graphOf(nodes, [connection('c1', 'n1_battery', 'n2_controller', 'power')])).graph;
}

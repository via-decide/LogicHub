...
function collectAffectedAreas(
  ...
  if (supplyChanged) {
    ...
    areas.push({
      area: 'Motor speed',
      domain: 'electrical',
      // A DC motor's speed tracks the voltage applied to it, but this release
      // derives speed from the nameplate no-load figure alone. The area is
      // genuinely affected and the platform cannot yet say by how much.

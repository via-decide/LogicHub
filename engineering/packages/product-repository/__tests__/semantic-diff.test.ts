...
describe('Gate 8 — semantic diff', () => {
  ...
  it('does not claim a motor speed change this release cannot derive', () => {
    // Motor speed physically tracks applied voltage, but this release derives
    // it from the nameplate no-load figure alone. Reporting a speed change
    // here would be inventing a number the model never computed.
    const { diff } = repoWithBatteryChange();
    expect(diff.changes.map(c => `${c.nodeType}.${c.field}`)).not.toContain('motor.speedMps');
  });
  ...
  it('lists motor speed as affected even though it cannot quantify the change', () => {
    // Listing it unevaluated is the honest middle ground: the area really is
    // reached, and the platform really cannot say by how much.
    const { diff } = repoWithBatteryChange();
    const speed = area(diff, 'Motor speed');

    expect(speed.evaluated).toBe(false);
    expect(speed.effect).toBeNull();

...
let html = source.replaceAll('__LOGICHUB_BUILD_ID__', sourceHash).replaceAll('__DEMO_BUNDLE_SHA256__', bundleHash);
...
if (existsSync('site/how-it-works.html')) {
  let hiw = readFileSync('site/how-it-works.html', 'utf8').replaceAll('__LOGICHUB_BUILD_ID__', sourceHash);
...
console.log(JSON.stringify({ buildId: sourceHash, rootSha256: createHash('sha256').update(html).digest('hex'), bundleHash }, null, 2));

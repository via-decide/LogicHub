...
/* Underlined, not merely orange. A link inside a paragraph that differs from the
   text by colour alone is invisible to anyone who cannot see that difference. */
...
/* A filled button is distinguishable by its own shape and background, so it does
   not need the underline the inline links carry. */
...
into the form. Collected only when you submit it. We email you once to confirm, and
you are not counted as being on the list until you click that link.</dd>
<dt>Abuse counters</dt><dd>A one-way hash of your network address, held for one hour
so the signup form cannot be flooded. The address itself is not stored, and the
counter records nothing about who you are or what you looked at.</dd>
...
remove you, whichever comes first. An entry you never confirm is never written to
again beyond the single confirmation email.</dd>
<dt>Abuse counters</dt><dd>One hour.</dd>
...
<h2>Join the list</h2>
<p>We send one email asking you to confirm, and you are not on the list until you
click the link in it. Anyone can type anyone's address into a form, so a signup on
its own is not consent from the person it names.</p>
